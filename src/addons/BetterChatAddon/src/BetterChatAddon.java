...
public class BetterChatAddon implements Addon, Listener {

	private ColoredGroups cg = null;
	...
	public void enable() {
		...
		try {
			/*Hook into ColoredGroups*/
			Plugin c = Bukkit.getPluginManager().getPlugin("ColoredGroups");
			if (c != null) {
				cg = (ColoredGroups) c;
			} else {
				System.out.println("Could not hook into ColoredGroups");
			}
			/*Disable the default chat listener*/
			AsyncPlayerChatEvent.getHandlerList().unregister(new ChatHandler(cg));
			/*Enable the new chat listener */
			cg.getServer().getPluginManager().registerEvents(this, cg);
		} catch (Exception exe) {
	...
	public void disable() {
		...
		cg.getServer().getPluginManager().registerEvents(new ChatHandler(cg), cg);
